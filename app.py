...
# DATABASE CONNECTION
# ===========================
# Render: set DATABASE_URL to the Render PostgreSQL External Database URL.
# Local: without DATABASE_URL, the existing MySQL .env connection is used.
...
class _CursorAdapter:
    ...
    def _pg_sql(self, sql):
        sql=re.sub(r'\bAUTO_INCREMENT\b','GENERATED BY DEFAULT AS IDENTITY',sql,flags=re.I)
        sql=re.sub(r'\bDATETIME\b','TIMESTAMP',sql,flags=re.I)
        sql=re.sub(r'\s+AFTER\s+[A-Za-z_][A-Za-z0-9_]*','',sql,flags=re.I)
        sql=re.sub(r'information_schema\.COLUMNS','information_schema.columns',sql,flags=re.I)
        sql=re.sub(r'TABLE_SCHEMA\s*=\s*DATABASE\(\)','table_schema = current_schema()',sql,flags=re.I)
        sql=re.sub(r'\bTABLE_NAME\b','table_name',sql,flags=re.I)
        sql=re.sub(r'\bCOLUMN_NAME\b','column_name',sql,flags=re.I)
        sql=re.sub(r'CAST\(([^()]+)\s+AS\s+CHAR\)',r'CAST(\1 AS TEXT)',sql,flags=re.I)
        sql=sql.replace("GROUP_CONCAT(", "STRING_AGG(")
        sql=re.sub(r"DATE_FORMAT\(([^,]+),\s*'%d-%m-%Y'\)",r"TO_CHAR(\1, 'DD-MM-YYYY')",sql,flags=re.I)
        sql=re.sub(r"STRING_AGG\(\s*CASE\s+WHEN\s+(.+?)\s+THEN\s+TO_CHAR\(([^,]+),\s*'DD-MM-YYYY'\)\s+END\s+ORDER BY\s+(.+?)\s+SEPARATOR\s*'?,\s*'?\s*\)",
                   r"STRING_AGG(CASE WHEN \1 THEN TO_CHAR(\2, 'DD-MM-YYYY') END, ', ' ORDER BY \3)",sql,flags=re.I|re.S)
        return sql
    def execute(self,operation,params=None):
        if self._pg: operation=self._pg_sql(operation)
        self._cur.execute(operation,params)
        if self._pg and re.match(r'\s*INSERT\s+INTO\b',operation,re.I):
            try:
    ...
    def executemany(self,operation,seq):
        if self._pg: operation=self._pg_sql(operation)
        self._cur.executemany(operation,seq)
        return self
...
class _DBAdapter:
    def __init__(self):
        self.is_pg=bool(os.environ.get("DATABASE_URL"))
        self._connect()
    def _connect(self):
        if self.is_pg:
    ...
    def cursor(self,dictionary=False,buffered=False,**kwargs):
        if self.is_pg:
            cur=self._conn.cursor(cursor_factory=RealDictCursor if dictionary else None)
            return _CursorAdapter(cur,pg=True)
        return _CursorAdapter(self._conn.cursor(dictionary=dictionary,buffered=buffered,**kwargs),pg=False)
    def commit(self): self._conn.commit()
    def rollback(self): self._conn.rollback()
    def reconnect(self,attempts=3,delay=1):
        if self.is_pg:
            if self._conn.closed: self._connect()
        else: self._conn.reconnect(attempts=attempts,delay=delay)
